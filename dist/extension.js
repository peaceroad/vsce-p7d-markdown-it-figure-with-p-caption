...
var require_shared = __commonJS({
  "node_modules/sync-fetch/shared.js"(exports2, module2) {
    var Stream = require("stream");
    function serializeHeaders(headers) {
      return headers.raw();
    }
    function deserializeHeaders(fetch2, old = {}) {
      const headers = new fetch2.Headers();
    ...
    function deserializeRequest(fetch2, resource, init) {
      return new fetch2.Request(resource, {
        ...init,
        body: init.body ? Buffer.from(init.body, "base64") : void 0,
        headers: deserializeHeaders(fetch2, init.headers)
      });
    }
    function serializeResponse(response) {
      return {
        ...
        // could be more than one, but no way of telling
      };
    }
    function deserializeResponse(fetch2, body, init, bodyError) {
      const buffer = Buffer.from(body, "base64");
      return new fetch2.Response(
    ...
    function deserializeError(fetch2, name, init) {
      if (name in errors) {
        return new errors[name](...init);
      } else {
        return new fetch2.FetchError(...init);
      }
    }
...
var require_sync_fetch = __commonJS({
  "node_modules/sync-fetch/index.js"(exports2, module2) {
    var exec = require("child_process").execFileSync;
    var path2 = require("path");
    var { URL } = require("url");
    var _fetch = require_lib2();
    ...
    function fetch2(resource, init) {
      ...
      if ("headers" in response[1]) {
        return shared.deserializeResponse(fetch2, ...response);
      } else {
        throw shared.deserializeError(fetch2, ...response);
      }
    }
    function sendMessage(message) {
      return exec(process.execPath, ["C:\\Users\\k_taka\\git\\github.com\\vsce-p7d-markdown-it-figure-with-p-caption\\node_modules\\sync-fetch\\worker.js"], {
        windowsHide: true,
        maxBuffer: Infinity,
    ...
    var SyncRequest = class _SyncRequest extends _fetch.Request {
      ...
      clone() {
        checkBody(this);
        return new _SyncRequest(...shared.serializeRequest(this));
      }
    };
    var SyncResponse = class _SyncResponse extends _fetch.Response {
    ...
    var Body = class _Body {
      ...
      json() {
        ...
        try {
          ...
          throw new fetch2.FetchError(`invalid json response body at ${this.url} reason: ${err.message}`, "invalid-json");
        }
      }
      buffer() {
        checkBody(this);
      ...
      textConverted() {
        throw new fetch2.FetchError("textConverted not implemented");
      }
    };
    ...
    function defineBodyError(body, error) {
      Object.defineProperty(body, _bodyError, {
        value: shared.deserializeError(fetch2, ...error),
        enumerable: false
      });
    }
    ...
    fetch2.Headers = _fetch.Headers;
    fetch2.FetchError = _fetch.FetchError;
    fetch2.Request = SyncRequest;
    fetch2.Response = SyncResponse;
...
// node_modules/@peaceroad/markdown-it-renderer-fence/index.js
var fenceStartTag = (tagName, sAttr) => {
...
var getFenceHtml = (tokens, idx, env, slf, md, options) => {
  const opt = {
    setHighlight: true,
    setLineNumber: true,
...
var mditRendererFence = (md, options) => {
  md.renderer.rules["fence"] = (tokens, idx, env, slf) => {
    return getFenceHtml(tokens, idx, env, slf, md, options);
  };
};
var markdown_it_renderer_fence_default = mditRendererFence;

// node_modules/@peaceroad/markdown-it-renderer-image/index.js
var import_path = __toESM(require("path"), 1);
var import_sync_fetch = __toESM(require_sync_fetch(), 1);
var import_image_size = __toESM(require_dist(), 1);
var setImgSize = (token, img, imgData, option) => {
  if (!imgData) return token;
  let w = imgData.width;
  let h = imgData.height;
  const imgName = import_path.default.basename(img, import_path.default.extname(img));
...
var addAsyncDecode = (imgCont) => {
  imgCont = imgCont.replace(/( *?\/)?>$/, ' decoding="async"$1>');
  return imgCont;
};
var addLazyLoad = (imgCont) => {
  imgCont = imgCont.replace(/( *?\/)?>$/, ' loading="lazy"$1>');
  return imgCont;
};
var setLocalImgSrc = (imgSrc, option, env) => {
  let img = "";
  if (option.mdPath) {
    img = import_path.default.dirname(option.mdPath);
  } else {
    if (env !== void 0) {
      if (env.mdPath) {
        img = import_path.default.dirname(env.mdPath);
      }
    }
  }
...
var mditRendererImage = (md, option) => {
  const opt = {
    scaleSuffix: false,
    mdPath: "",
    lazyLoad: false,
  ...
  md.renderer.rules["image"] = (tokens, idx, options, env, slf) => {
    ...
    if (isNotLocal) {
      try {
        const response = (0, import_sync_fetch.default)(imgSrc);
...
var markdown_it_renderer_image_default = mditRendererImage;

// extension.js
var import_markdown_it_attrs = __toESM(require_markdown_it_attrs());

// node_modules/@peaceroad/markown-figure-num-setting/index.js
var setMarkdownFigureNum = (markdown, option) => {
  let opt = {
    img: true,
    table: false,
    code: false,
  ...
  const label = (hasMarkLabel, counter2, isAlt) => {
    ...
    let LabelIsEn = /[a-zA-Z]/.test(label2);
  ...
  const setImageAltNumber = (lines2, n2, mark, hasMarkLabel, counter2) => {
    ...
    while (i >= 0) {
      ...
      isFigureImage = lines2[i].match(new RegExp(figureImageReg));
      ...
      while (j >= 0) {
        ...
        if (isFigureImage[3] === (counter2.img - 1).toString()) {
          hasPrevFigureImage = false;
          break;
        }
        lines2[i] = lines2[i].replace(new RegExp(figureImageReg), "$1" + label(hasMarkLabel, counter2[mark], true) + "$4");
...
var markown_figure_num_setting_default = setMarkdownFigureNum;

// node_modules/@peaceroad/markdown-imgattr-to-pcaption/index.js
var setMarkdownImgAttrToPCaption = (markdown, option) => {
  const opt = {
    imgAltCaption: true,
    imgTitleCaption: false,
    labelLang: "ja"
...
var markdown_imgattr_to_pcaption_default = setMarkdownImgAttrToPCaption;

// extension.js
var import_vscode = __toESM(require("vscode"));
var config = import_vscode.workspace.getConfiguration("p7dMarkdownItFigureWithPCaption");
...
function activate(context) {
  ...
  import_vscode.workspace.onDidChangeConfiguration((event) => {
    ...
    if (event.affectsConfiguration("p7dMarkdownItFigureWithPCaption.disableStyle")) {
      disableStyle = import_vscode.workspace.getConfiguration("p7dMarkdownItFigureWithPCaption").get("disableStyle");
      if (disableStyle) {
  ...
  const registerCommand = (commandName, transformFunction, options, context2) => {
    if (registeredCommands[commandName]) {
      registeredCommands[commandName].dispose();
      delete registeredCommands[commandName];
    }
    let command = import_vscode.commands.registerCommand(commandName, () => {
      ...
      editor.edit((editBuilder) => {
        let fullRange = new import_vscode.default.Range(
    ...
    context2.subscriptions.push(command);
    registeredCommands[commandName] = command;
  };
  const setCommands = (exOption2) => {
    registerCommand("p7dMarkdownItFigureWithPCaption.setImgAltAttrToPCaption", markdown_imgattr_to_pcaption_default, { labelLang: exOption2.labelLang }, context);
    registerCommand("p7dMarkdownItFigureWithPCaption.setImgTitleAttrToPCaption", markdown_imgattr_to_pcaption_default, { imgTitleCaption: true, labelLang: exOption2.labelLang }, context);
  };
  let pat = "";
  if (import_vscode.window.activeTextEditor && import_vscode.window.activeTextEditor.document.uri.scheme === "file") {
    pat = import_vscode.window.activeTextEditor.document.uri.fsPath;
  }
  registerCommand("p7dMarkdownItFigureWithPCaption.setFigureCaptionNumber", markown_figure_num_setting_default, { noSetAlt: true }, context);
  ...
  const expandMarkdownItSetting = (md) => {
    if (exOption.setImageElementAttributes) {
      md.use(markdown_it_renderer_image_default, {
        scaleSuffix: true,
        resize: true,
        lazyLoad: true,
        asyncDecode: false,
    ...
    md.use(markdown_it_figure_with_p_caption_default, {
